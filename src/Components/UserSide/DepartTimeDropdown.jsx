...
const DepartTimeDropdown = ({ selectedTime, onChange, name, disabled, plateNumber, date, token }) => {
  console.log('DepartTimeDropdown - Received plateNumber:', plateNumber);
  console.log('DepartTimeDropdown - Received date:', date);
  ...
  const isReserved = (time) => {
    ...
    const reserved = reservedTimes.some(({ departureTime, pickUpTime }) => {
      const formattedDeparture = departureTime.replace(':', ''); 
